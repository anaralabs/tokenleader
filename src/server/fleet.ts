...
// --- thresholds --------------------------------------------------------------
// The daemon ticks (and now checks in) every 5 minutes; the watchdog fires
// every 120s of awake time. Every threshold errs toward the daemon's normal
// rhythms — sleep, slow links, one dropped request — never flagging them.
 ...
 *  (an asleep machine silences BOTH channels). */
 ...
 *  an unchanged heartbeat (8 × 120s ≈ 16 min of awake time). */
 ...
 *  CRASH_LOOPING and HEARTBEAT_IO surface in /stats/fleet but page nobody
 *  yet — the local fuse (degraded marker) escalates them to DEGRADED. */
export const ALERT_STATES: ReadonlySet<FleetState> = new Set(["WEDGED", "DEGRADED", "DARK"]);
...
/** Coerce an untrusted /checkin JSON body into a CheckinBody (defaults for
 *  anything missing or mistyped), or null when it isn't an object at all. */
export function sanitizeCheckinBody(raw: unknown): CheckinBody | null {
...
export function classifyDevice(s: DeviceSignals, now: number): FleetState {
  ...
  const daemonFresh = s.lastSeen !== null && daemonSilenceMs <= DAEMON_LATE_MS;
  ...
  const staleHeartbeat =
    watchdogFresh && (s.watchdog?.heartbeat_age_runs ?? 0) >= STALE_HEARTBEAT_RUNS;
  ...
  if (hasWatchdog) {
    if (watchdogFresh) {
      ...
      if (staleHeartbeat || daemonSilenceMs > DAEMON_WEDGE_MS) return "WEDGED";
      return "LATE";
    }
 ...
 * how long it has held its state, and for WEDGED/DEGRADED/DARK held > 1h,
 * queue the heal (WEDGED → device-targeted restart, deduped so a
 * non-recovering machine isn't restart-stormed) and page the webhook (6h
 * dedup per device+state, persisted so a redeploy doesn't re-page the
 * fleet). Every decision is logged; failures never throw.
 */
export async function sweepFleetAlerts(opts: SweepOptions): Promise<FleetAlertDecision[]> {
  ...
  for (const { user, device } of opts.store.listFleetDevices()) {
    const state = classifyDevice(collectDeviceSignals(opts.store, device), now);
    ...
    const push = (action: FleetAlertDecision["action"], healQueued: boolean): void => {
      decisions.push({ user, deviceId: device.id, label: device.label, state, action, healQueued });
    };
    ...
    // without recovering isn't hammered every sweep.
    ...
    if (!opts.webhookUrl) {
      log(
        `[tokenleader] fleet sweep: ${name} is ${state} — no TOKENLEADER_ALERT_WEBHOOK, not paging`,
      );
      push("no_webhook", healQueued);
      continue;
    }
    const lastAlert = opts.store.lastFleetAlertAt(device.id, state);
    if (lastAlert !== null && now - lastAlert < ALERT_DEDUP_MS) {
      log(
        `[tokenleader] fleet sweep: ${name} still ${state} — already alerted ${fmtAgo(now, lastAlert)}, deduped`,
      );
      push("deduped", healQueued);
      continue;
    }
    ...
    try {
      ...
      delivered = res.ok;
      if (!res.ok) {
        log(`[tokenleader] fleet sweep: webhook returned ${res.status} for ${name} ${state}`);
      }
    } catch (err) {
      log(`[tokenleader] fleet sweep: webhook POST failed for ${name} ${state}: ${err}`);
    }
    if (delivered) {
      // Recorded only on success so a transient webhook failure retries
      // next sweep instead of being silently deduped for 6h.
      opts.store.recordFleetAlert(device.id, user, state, now);
      log(`[tokenleader] fleet sweep: alerted ${name} ${state}`);
      push("alerted", healQueued);
    } else {
      push("webhook_failed", healQueued);
    }
  }
  return decisions;
}
